  ...
  return (
    <section ref={containerRef} className="relative h-screen w-full overflow-hidden bg-[#050505]">
      ...
      <motion.div style={{ opacity, scale }} className="relative z-10 h-full flex flex-col justify-between p-8 pt-32 md:p-12 md:px-12 md:pt-40 md:pb-20">
        ...
        >
          <p className="font-mono text-xs tracking-widest text-muted-foreground mb-4 max-w-xs mx-auto">{company.tagline}</p>
        ...
        >
          <p className="font-mono text-xs tracking-[0.3em] text-muted-foreground mb-2">02 — SINCE {company.establishedYear}</p>
          <h2 className="font-sans text-4xl md:text-6xl lg:text-7xl font-light tracking-tight text-balance">
            {company.brandName.split(" ")[0]}
            <br />
            <span className="italic">{company.brandName.split(" ").slice(1).join(" ") || "Technologies"}</span>
          </h2>
        </motion.div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5 }} className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10">
        <motion.div animate={{ y: [0, 8, 0] }} transition={{ duration: 1.5, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut" }} className="flex flex-col items-center gap-2">
